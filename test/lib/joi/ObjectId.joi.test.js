...
describe('lib/joi', () => {
  describe('#ObjectId', () => {
    ...
    it('should throw with an invalid ObjectID', () => {
      ...
      expect(res.error).to.have.property('message', '"value" must be a valid ObjectID');
    });

    it('should validate with a valid ObjectID as string', () => {
      const res = Joi.attempt('584568fc413029a2d7a7a3cf', schema);
      expect(res).to.deep.equal(ObjectID.createFromHexString('584568fc413029a2d7a7a3cf'));
    });

    it('should validate with a valid ObjectID as ObjectID', () => {
      const res = Joi.attempt(ObjectID.createFromHexString('584568fc413029a2d7a7a3cf'), schema);
      expect(res).to.deep.equal(ObjectID.createFromHexString('584568fc413029a2d7a7a3cf'));
    });
  });
});
